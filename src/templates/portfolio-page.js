...
export class PortfolioPageTemplate extends Component {
  ...
  render() {
    ...
    function handlePortfolioTypeClick(type) {
      const storageOpenPortfolioType =
        sessionStorage.getItem('openPortfolioType')
      const openPortfolioType = type !== storageOpenPortfolioType ? type : ''
      sessionStorage.setItem('openPortfolioType', openPortfolioType)
    }
    const imageGallery =
      this.selectedJob.imageGallery &&
      this.selectedJob.imageGallery.length > 0 ? (
        <ImageGallery
          key={this.selectedJob.name}
          images={this.selectedJob.imageGallery}
        />
      ) : (
        <ImageGallery
            ...
            <dd className="portfolio-type">
              {page.frontmatter.portfolioTypes.map((portfolioType, index) => (
                <dl key={index} className="portfolio-list-item">
                  <dt>
                    <Link
                      to={`/portfolio/${portfolioType.name
                        .replace(/\s/g, '-')
                        .toLowerCase()}`}
                      state={{ portfolioType, selectedJob: this.selectedJob }}
                      onClick={() =>
                        handlePortfolioTypeClick(portfolioType.name)
                      }
                    >
                      {portfolioType.name}
                    </Link>
                  ...
                  {portfolioType.jobs.map((job, index) => (
                    <dd
                      key={index}
                      className={
                        portfolioType.name === openPortfolioType
                          ? 'portfolio-jobs open'
